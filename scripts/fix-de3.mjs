// Fix German translations in SUPPLEMENTAL object only
...
if (suppStart === -1) { console.error('SUPPLEMENTAL not found'); process.exit(1) }
...
for (let i = 0; i < lines.length; i++) {
  for (const [key, value] of deTranslations) {
    ...
    if (lines[i].match(regex)) {
      lines[i] = lines[i].replace(regex, `$1'${value}'$3`)
      updated++
      break
    }
  }
