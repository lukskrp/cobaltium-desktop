...
// Process line by line - line 9 has multiple entries on same line
// Key fix: use [^{]*\\{[^}]* to match through the entire { ... } block
...
for (let i = 0; i < lines.length; i++) {
  ...
  for (const [key, value] of Object.entries(replacements)) {
    ...
    // Pattern: key... { ... de: 'value' ... }
    // [^{]*\\{ matches key: { and [^}]* matches up to closing }
    const regex = new RegExp(`(${escapedKey}:[^{]*\\{[^}]*de:\\s*')([^']*?)'([},])`, 'g')
    if (regex.test(line)) {
      line = line.replace(regex, `$1${value}'$3`)
      updated++
      console.log(`Updated: ${key} (line ${i})`)
    }
  }
