...
// Process line by line using a safer regex
// Key insight: use non-greedy [^}]*? to find de: within { ... }
...
for (let i = 0; i < lines.length; i++) {
  ...
  for (const [key, value] of Object.entries(replacements)) {
    ...
    // Pattern matches: key: { ... de: 'value' ... }
    // [^{]* matches before {, then \{, then [^}]*? non-greedily finds de:
