...
// Pattern to skip over { key: 'value', ... } pairs and find de: 'value'
// Match the key, then optionally skip langcode: 'value' pairs, find de: 'currentValue'
let updated = 0
for (const [key, value] of deTranslations) {
  const escapedKey = key.replace(/[.*+?^=(){}|[\]$]/g, '\\$&')
  // Non-greedy match that skips over language entries
