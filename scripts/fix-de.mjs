// Fix German translations in SUPPLEMENTAL object
...
for (const [key, value] of deTranslations) {
  keysFound++
  // Match the key in SUPPLEMENTAL, find the de: value, and replace it
  const regex = new RegExp(`(${key}: \\{[^}]*?de: ')[^}]+(\\})`, 'g')
...
console.log(`Found ${keysFound} keys, updated ${keysUpdated} keys.`)
