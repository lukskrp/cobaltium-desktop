...
for (const [key, value] of Object.entries(replacements)) {
  // Replace de: 'old' with de: 'new' on the line containing this key
  // The format varies: {de: 'value'.}, {other: '...', de: 'value'.}, {de: 'value'}
  const escapedKey = key.replace(/[.*+?^=(){}|[\]$]/g, '\\$&')
  // Find de: ' followed by content, capture to end but before closing quote
  const regex = new RegExp(`(${escapedKey}:.*?de:\\s*')([^']*')`, 'g')
  
  if (regex.test(suppContent)) {
    suppContent = suppContent.replace(regex, `$1${value}'`)
    updated++
