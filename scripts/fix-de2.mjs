// Fix German translations in SUPPLEMENTAL object only (after line ~640)
import { readFileSync, writeFileSync } from 'node:fs'

const filePath = 'D:\\Documents_D\\Cobaltium_Desktop\\scripts\\gen-locales.mjs'
let content = readFileSync(filePath, 'utf8')

// Only process content after 'const SUPPLEMENTAL'
const suppStart = content.indexOf('const SUPPLEMENTAL')
if (suppStart === -1) {
  console.error('SUPPLEMENTAL not found')
  process.exit(1)
}
...
const deTranslations = [
  ...
  ["'tour.llm.model.body'", "Welches Modell zu verwenden. Ein empfohlenes Standardmodell ist vorausgefüllt – Sie können es beibehalten."],
...
// For SUPPLEMENTAL entries, format is:
//   'key.name': { de: 'value', en: 'value' },
// We need to match the de: value within the curly braces
let updated = 0
for (const [key, value] of deTranslations) {
  // Escape special regex chars in key
  const escapedKey = key.replace(/[.*+?^=(){}|[\]$]/g, '\\$&')
  // Match: 'key.name': { ... de: 'value' ... }
  // Use a non-greedy match for the content between { and }
  const regex = new RegExp(`(${escapedKey}:\\s*\\{[^}]*?de:\\s*)'(.*?)'(\\s*\\})`, 'g')
  
  if (regex.test(suppContent)) {
    suppContent = suppContent.replace(regex, `$1'${value}'$3`)
    updated++
  }
}
...
console.log(`Updated ${updated} SUPPLEMENTAL entries.`)
